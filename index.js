...
const getSubscriber = async () => {
  try{
    ...
    for(const user of users){
      ...
      if(
        ![...subscribers].some((existingPair) => existingPair[0] === pair[0] && existingPair[1] === pair[1])
      ){
  ...
  try {
    ...
    for (const subscriber of subscribers) {
      if (subscriber.status) {
        ...
        console.log(`Weather update sent to subscriber: ${subscriber.chatId}`);
      }
    }
  } catch (error) {
...
bot.on("message", (msg) => {
  ...
  if(messageText === "/subscribe"){
    ...
    .then((response) => {
      console.log("Data sent successfully to localhost:3000", response.data);
  ...
  if(messageText === "/unsubscribe"){
    ...
    .catch((error) => {
      ...
      bot.sendMessage(chatId, "Failed to unsubscribe. Please try again later.");  
    });
  }
